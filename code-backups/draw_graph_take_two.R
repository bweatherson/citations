...
cited_years <- 1975:1995
citing_years <- 1975:1995
...
good_cite <- master_cite %>%
  ...
  filter(firstauth.old != firstauth.new)
...
# Leaving in same author pairs, because we got rid of self-cites
top_pairs <- all_pairs %>%
  ungroup() %>%
  group_by(X1, X2) %>%
  tally() %>%
...
g <- ggraph(cites_graph, layout = "nicely") + 
  geom_edge_link(width = 0.3, alpha = 0.5,
                 aes(color = linethick)) + 
...
ggsave("all_1975_1995.png", scale = 5, limitsize = FALSE)