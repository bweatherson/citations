...
edges_172 <- topic_connection %>% 
  filter(score > 0.05) %>% 
  filter(cite_from_cat_real_name %in% temp$cite_from_cat_real_name) %>% 
  filter(cite_to_cat_real_name %in% temp$cite_from_cat_real_name) %>% 
  mutate(weight = round(score, 1))


nodes_172 <- category_years %>% 
  filter(cat_real_name %in% temp$cite_from_cat_real_name) %>% 
